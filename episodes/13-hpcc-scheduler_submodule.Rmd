...
library(yaml)
config <- yaml.load_file("files/snippets_library/HPCC_MagicCastle_slurm/lesson_config.yaml")
snippets <- paste('files/snippets_library/', config$snippets, sep='')

#load local values 
if (is.character(config$local_snippets)) {
config <- yaml.load_file("files/snippets_library/EPCC_ARCHER2_slurm/lesson_config.yaml")
snippets <- paste('files/snippets_library/', config$local_snippets, sep='')
}
```
...
``r config$sched$flag$submit$name`` command.
...
```bash
`r config$remote$prompt`{{site.sched.del }} 38759
...
``r config$sched$flag$submit$name``. However, instead of specifying these in a script,
...
``r config$sched$flag$submit$name``.
